...
function installPolish(){
  if(document.getElementById('app-js-polish')) return;
  const style = document.createElement('style');
  style.id = 'app-js-polish';
  style.textContent = `
    .line:hover{background:transparent!important;box-shadow:none!important;border-left-color:transparent!important;border-right-color:transparent!important;border-top-color:transparent!important;}
    .line:hover .lineMain{border-color:color-mix(in srgb,var(--blue) 30%,var(--line))!important;background:rgba(255,255,255,.16)!important;box-shadow:0 8px 22px rgba(36,92,255,.045)!important;}
    .row[data-selected="true"]:not(.active){background:transparent!important;border-color:transparent!important;box-shadow:none!important;color:var(--ink)!important;}
    .row[data-selected="true"]:not(.active) b,.row[data-selected="true"]:not(.active) span{color:inherit!important;}
  `;
...
function visible(){ const q=S.q.toLowerCase(); return DB.statements.filter(s=>{ if(S.person && s.personId!==S.person)return false; if(S.concept && !s.concepts.includes(S.concept))return false; if(S.question && !s.questions.includes(S.question))return false; if(S.domain && !s.domains.includes(S.domain))return false; return !q || s.search.includes(q); }).sort(sortChrono); }
...
function renderRail(st){
  ...
  $('#rail').innerHTML=`<div class="section"><div class="label">Navigate</div><div class="navGrid">${nav()}</div></div><div class="section"><div class="subLabel">Controls</div><div class="tabs"><button class="chip" data-act="theme">Theme</button><button class="chip" data-act="reset">Reset</button></div></div><div class="section"><div class="subLabel">Filters</div><div class="tokens">${filters.join('')||'<span class="subLabel">none</span>'}</div></div><div class="section"><div class="subLabel">Sort</div><div class="tabs"><button class="chip ${S.sort==='author'?'active':''}" data-act="sort" data-val="author">author</button><button class="chip ${S.sort==='concept'?'active':''}" data-act="sort" data-val="concept">concept</button></div></div><div class="section"><div class="subLabel">Browse</div><div class="tabs"><button class="chip ${S.rail==='concepts'?'active':''}" data-act="rail" data-val="concepts">concepts</button><button class="chip ${S.rail==='questions'?'active':''}" data-act="rail" data-val="questions">questions</button><button class="chip ${S.rail==='domains'?'active':''}" data-act="rail" data-val="domains">domains</button><button class="chip ${S.rail==='authors'?'active':''}" data-act="rail" data-val="authors">authors</button></div></div>${railList(st)}`;
}
...
function groupsForSort(st){ return S.sort==='concept' ? groupByConcept(st) : groupByAuthor(st); }
...
function groupByConcept(st){ const seen = count(st.flatMap(s=>s.domains)); return seen.map(([d,n])=>({title:CAT_LABEL[d]||pretty(d), meta:`${n}`, items:st.filter(s=>s.domains.includes(d)).sort(sortChrono)})); }
