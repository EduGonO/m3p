...
function render(){ if(!DB)return; const st=visible(); $('#sub').textContent=positionLine(st); renderRail(st); $('#content').innerHTML=({atlas:viewAtlas,read:viewRead,routes:viewRoutes,pressure:viewPressure}[S.view]||viewAtlas)(st); if(S.view==='atlas') requestAnimationFrame(drawGraph); renderInspector(st); }
function positionLine(st){ const bits=[]; if(S.person) bits.push(DB.peopleById[S.person]?.name); if(S.concept) bits.push(pretty(S.concept)); if(S.question) bits.push(S.question); if(S.domain) bits.push(CAT_LABEL[S.domain]||pretty(S.domain)); return `${S.view} ${bits.length?'· '+bits.join(' · '):''} · ${st.length} claims · ${count(st.map(s=>s.person)).length} authors`; }
...
function claimLine(s, opts={}){ const expanded=S.expanded.has(s.id); return `<article class="line" data-act="inspectStatement" data-val="${s.id}"><button class="bullet" data-act="expand" data-val="${s.id}" aria-expanded="${expanded}">${expanded?'−':'+'}</button><div class="lineMain"><div class="lineTop"><button class="author inspectable" data-act="inspectPerson" data-val="${s.personId}">${esc(s.person)}</button><span class="date">${esc(s.time)} ${relBadgeText(s)}</span></div><div class="claimText">${esc(s.text)}</div><div class="meta">${semanticLine(s)}${opts.variant==='pressure'?`<span class="mono">${score(s)} links</span>`:''}</div>${expanded?expandedBlock(s):''}</div></article>`; }
function expandedBlock(s){ const plus=rels(s.id).filter(r=>r.type==='support').slice(0,6), minus=rels(s.id).filter(r=>r.type==='challenge').slice(0,6); return `<div class="expanded"><div class="relList">${plus.map(r=>relation(r,s.id)).join('')}${minus.map(r=>relation(r,s.id)).join('') || (!plus.length?'<p>no links</p>':'')}</div></div>`; }
...
function inspectStatement(id){ const s=DB.byId[id]; if(!s)return; $('#context').innerHTML=`<div class="card"><div class="lineTop"><h3>${esc(s.person)}</h3><span class="date">${esc(s.time)}</span></div><p>${esc(s.summary)} · ${esc(s.movement)}</p><div class="claimText" style="margin-top:8px">${esc(s.text)}</div><div class="meta">${s.domains.map(d=>domain(d,true)).join('')}${s.concepts.map(c=>`<button class="mono inspectable" data-act="inspectConcept" data-val="${esc(c)}">${esc(pretty(c))}</button>`).join('')}${s.questions.slice(0,3).map(q=>`<button class="question inspectable" data-act="inspectQuestion" data-val="${esc(q)}">${esc(q)}</button>`).join('')}</div></div><div class="card" style="margin-top:10px"><div class="subLabel">+</div><div class="relList">${rels(id).filter(r=>r.type==='support').map(r=>relation(r,id)).join('')||'<p>None.</p>'}</div></div><div class="card" style="margin-top:10px"><div class="subLabel">−</div><div class="relList">${rels(id).filter(r=>r.type==='challenge').map(r=>relation(r,id)).join('')||'<p>None.</p>'}</div></div>`; }
